...
from app.src.domain.models import Bot, User, SubPlains, StatusBot, BotStore, PendingChatId
...
async def setup(request: Request, bg: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    # Resolve os objetos Store pelo brand
    store_repo = StoreRepository(db)
    stores     = store_repo.get_by_brands(brands)
    if not stores:
        return JSONResponse({"error": "Nenhuma loja válida encontrada."}, status_code=400)

    # Remove o pending após uso
    ...
    if bot:
        ...
        db.add(bot)
        db.flush()  # gera bot.id antes de criar BotStore
        bot_repo.set_stores(bot, stores)
    ...
    # Passa os brands para o background task
    bg.add_task(afilibot, chat_id, brands, token, affiliate_link)
    return {"ok": True}